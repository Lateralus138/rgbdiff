...
static bool HelpRegex(const std::string &str) {
  return std::regex_match(str, HELPREGEX);
}
std::regex MINUREGEX("^-([mM]|-[mM][iI][nN][uU][eE][nN][dD])$");
static bool MinuRegex(const std::string &str) {
  return std::regex_match(str, MINUREGEX);
}
std::regex VERBREGEX("^-([vV]|-[vV][eE][rR][bB][oO][sS][eE])$");
static bool VerbRegex(const std::string &str) {
  return std::regex_match(str, VERBREGEX);
}
std::regex MINIREGEX("^-([nN]|-[mM][iI][nN][iI][mM][uU][mM])$");
static bool MiniRegex(const std::string &str) {
  return std::regex_match(str, MINIREGEX);
}
std::regex MAXIREGEX("^-([xX]|-[mM][a][xX][iI][mM][uU][mM])$");
static bool MaxiRegex(const std::string &str) {
  return std::regex_match(str, MAXIREGEX);
}
// static long long
// CheckIfStringIsHexAndConvertToLongThenCheckIfIs24BitValue(std::string& value)
static long long CheckIfStringIsHexAndConvertToLongThenCheckIfIs24BitValue(
    std::string_view value) {
  if (!Globals::IsHexadecimalString(value))
    return static_cast<long long>(-2);
  long long iValue =
      static_cast<long long>(std::stoll(std::string(value), NULL, 16));
  if ((iValue < 0x000000) || (iValue > 0xffffff))
    return static_cast<long long>(-3);
  return iValue;
}

static int minuendCheckAndAdd(std::string_view value, Options &options) {
  long long check =
      CheckIfStringIsHexAndConvertToLongThenCheckIfIs24BitValue(value);
  if (check >= 0)
    options.minuend = check;
...
static int subtrahendsCheckAndAdd(std::string &value, Options &options) {
  long long check =
      CheckIfStringIsHexAndConvertToLongThenCheckIfIs24BitValue(value);
  if (check >= 0)
    options.subtrahends.push_back(check);
  return static_cast<int>((check < 0) ? (check * -1) : 0);
}
...
static int ParseArguments(ArgumentParser &args, Options &options) {
  if (args.optionExistsByPredicate(HelpRegex, true)) {
    std::string message =
        ...
        "\n                  this value is used, otherwise the first value "
        "passed"
        "\n                  to the program is used."
        "\n\n  @SWITCHES"
        "\n    -h, --help    This help message."
        "\n    -v, --verbose Detailed output."
        "\n    -n, --minimum Return the minimum value only if multiple "
        "subtrahends"
        "\n                  are passed."
        "\n    -x, --maximum Return the maximum value only if multiple "
        "subtrahends"
        "\n                  are passed."
        "\n                  If both of the minimum and maximum switches are "
        "passed"
...
int main(int argc, const char *argv[]) {
  ...
  Errors errors = {{{1, "Not enough arguments passed to the program."},
                    {2, "Value is not a valid hexadecimal value."},
                    {3, "Hexadecimal value is not a valid 24 bit color "
                        "(0x000000 - 0xFFFFFF, 0-16777215)."}}};
  if (argc < 2) {
  ...
  const std::string minuendString =
      Globals::IntToHexString(static_cast<int>(options.minuend), 6, "0x");
  for (const long long &number : options.subtrahends) {
    const std::string subtrahendString =
        Globals::IntToHexString(static_cast<int>(number), 6, "0x");
  ...
  double minOrMax = 0;
  if (options.minimum) {
    auto min = std::min_element(std::begin(diffValues), std::end(diffValues),
  ...
  if (options.maximum) {
    auto max = std::max_element(std::begin(diffValues), std::end(diffValues),
                                [](const auto &left, const auto &right) {
                                  return left.second < right.second;
                                });
    minOrMax = max->second;
